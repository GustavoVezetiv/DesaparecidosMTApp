...
export const personService = {
  ...
  async submitInformation(data: InformationSubmission): Promise<void> {
    try {
      ...
      if (!data.observacoes.trim() && !data.localizacao.trim()) {
        throw new Error('Pelo menos observações ou localização devem ser preenchidas');
      }
      ...
      if (data.fotos && data.fotos.length > 0) {
        ...
        for (const file of data.fotos) {
          if (file.size > maxFileSize) {
            throw new Error(`Arquivo ${file.name} é muito grande. Máximo 5MB por arquivo.`);
          }
          
          if (!allowedTypes.includes(file.type)) {
            throw new Error(`Tipo de arquivo ${file.type} não permitido. Use apenas JPEG, PNG ou GIF.`);
          }
        }
      }
      ...
      if (error.message.includes('validação') || error.message.includes('obrigatório')) {
